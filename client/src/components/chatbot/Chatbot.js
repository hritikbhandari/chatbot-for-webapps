...
class Chatbot extends Component {
    ...
    async df_text_query (text) {
        ...
        says = {
            ...
            msg: {
                text : {
                    text: "I'm having troubles. I need to terminate. will be back soon"
                }
            }
        }
        this.setState({ messages: [...this.state.messages, says]});
    ...
    async componentDidMount() {
        ...
        if (window.location.pathname === '/blog' && !this.state.shopWelcomeSent) {
            await this.resolveAfterXSeconds(1);
            this.df_event_query('WELCOME_BLOG');
            this.setState({ shopWelcomeSent: true, showBot: true });
        }

        this.props.history.listen(() => {
            if (this.props.history.location.pathname === '/blog' && !this.state.shopWelcomeSent) {
                this.df_event_query('WELCOME_BLOG');
                this.setState({ shopWelcomeSent: true, showBot: true });
            }
        });
    ...
    _handleQuickReplyPayload(event, payload, text) {
        ...
        switch (payload) {
            case 'recommended_yes':
                ...
                break;
            case 'training_readmore':
                this.df_event_query('READMORE');
                break;
            default:
